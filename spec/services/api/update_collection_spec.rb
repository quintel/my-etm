...
RSpec.describe Api::UpdateCollection do
  let(:user) { create(:user) }
  let(:result) { described_class.new.call(collection:, params:) }
  ...
  context 'with valid parameters, without scenario IDs' do
    ...
    it 'leaves the scenario IDs unchanged' do
      expect { result }.not_to(change { collection.scenarios.map(&:scenario_id).sort })
    end
  end
